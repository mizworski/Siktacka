...
Event::Event(uint32_t len, uint32_t event_no, char event_type) : len_(len),
                                                                 event_no_(event_no),
...
uint32_t Event::get_control_sum(std::string const &message) {
    boost::crc_32_type result;
    result.process_bytes(message.data(), message.length());
    return (uint32_t) result.checksum();
}

bool Event::check_control_sum(uint32_t other_sum) {
    std::string message(serialize());
...
uint32_t Event::get_size() {
    return len_ + 4;
}

int8_t Event::get_type() {
    return -1;
}

uint32_t Event::get_event_no() {
    return event_no_;
}

NewGame::NewGame(uint32_t event_no, uint32_t maxx, uint32_t maxy, std::vector<std::string> &players) : Event(17,
                                                                                                             event_no,
                                                                                                             0),
                                                                                                       maxx_(maxx),
                                                                                                       maxy_(maxy),
...
NewGame::NewGame(std::string const &serialized_message) {
...
int8_t NewGame::get_type() {
    return 0;
}

uint32_t NewGame::get_maxx() {
    return maxx_;
}

uint32_t NewGame::get_maxy() {
    return maxy_;
}

std::vector<std::string> NewGame::get_players() {
    return players_;
}

Pixel::Pixel(uint32_t event_no, char player_number, uint32_t x, uint32_t y) : Event(18, event_no, 1),
                                                                              player_number_(player_number),
                                                                              x_(x),
                                                                              y_(y) {}

Pixel::Pixel(std::string const &serialized_message) {
...
int8_t Pixel::get_type() {
    return 1;
}

char Pixel::get_player_number() {
...
std::string Pixel::get_string() {
    std::string res("PIXEL " + std::to_string(x_) + " " + std::to_string(y_) + " ");

    return res;
}

PlayerEliminated::PlayerEliminated(uint32_t event_no, char player_number) : Event(10, event_no, 2),
                                                                            player_number_(player_number) {}

PlayerEliminated::PlayerEliminated(std::string const &serialized_message) {
...
std::string PlayerEliminated::serialize() {
    std::ostringstream os;
    std::string message(Event::serialize());
...
std::string PlayerEliminated::get_string() {
    std::string res("PLAYER_ELIMINATED ");

    return res;
}

int8_t PlayerEliminated::get_type() {
    return 2;
}

char PlayerEliminated::get_player_number() {
    return player_number_;
}

GameOver::GameOver(uint32_t event_no) : Event(9, event_no, 3) {}

GameOver::GameOver(std::string const &serialized_message) {
...
int8_t GameOver::get_type() {
    return 3;
}

ServerDatagram::ServerDatagram(uint32_t game_id, std::vector<std::shared_ptr<Event>> &events) : game_id_(game_id),
                                                                                                events_(events) {}

ServerDatagram::ServerDatagram(std::string const &serialized_message) {
...
std::string ServerDatagram::serialize() {
    std::ostringstream os;
...
uint32_t ServerDatagram::get_game_id() {
    return game_id_;
}

std::vector<std::shared_ptr<Event>> ServerDatagram::get_events() {
    return events_;
}

ServerMessage::ServerMessage(uint32_t game_id) : game_id_(game_id) {}

ServerMessage::ServerMessage(uint32_t game_id, std::vector<std::shared_ptr<Event>> const &events) : game_id_(game_id),
                                                                                                    events_(events) {}

ServerMessage::ServerMessage(uint32_t game_id, std::vector<std::shared_ptr<Event>> &&events) : game_id_(game_id),
                                                                                               events_(std::move(
                                                                                                       events)) {}

ServerMessage::ServerMessage(uint32_t game_id, std::shared_ptr<Event> &event) : game_id_(game_id), events_() {
    events_.push_back(event);
}
...
void ServerMessage::push_back(std::shared_ptr<Event> &&event) {
    events_.push_back(std::move(event));
}

std::vector<ServerDatagram> ServerMessage::get_datagrams() {
